...
import tianchi_densenet201_master.model_densenet201 as l1
...
def main():
    ...
    def train(train_loader, model, criterion, optimizer, epoch):
        ...
        for i, (images, target) in enumerate(train_loader):
            ...
            x2=model2(image_var)
            x3=model3(image_var) 
            y_pred = model(torch.cat((x1, x2, x3), 1))
            # 计算loss
            ...
            teml=target.tolist()
            temp=float_turn_int(y_pred).tolist()
            
            y_actu_all.extend(teml)
            y_comp_all.extend(temp)
            f1_local=f1_score(teml, temp, average='weighted')
            f1_global=f1_score(y_actu_all, y_comp_all, average='weighted')
            ...
            if i % print_freq == 0:
                print('Epoch: [{0}][{1}/{2}]\t'
                      'F1 {f1_local:.3f} ({f1_global:.3f})\t'
                      'Loss {loss.val:.4f} ({loss.avg:.4f})\t'
                      'Accuray {acc.val:.3f} ({acc.avg:.3f})'.format(
                    epoch, i, len(train_loader), f1_local=f1_local,f1_global=f1_global, loss=losses, acc=acc))
        ...
        for i, (images, labels) in enumerate(val_loader):
            ...
            if i % print_freq == 0:
                print('TrainVal: [{0}/{1}]\t'
                      ...
                      'Loss {loss.val:.4f} ({loss.avg:.4f})\t'
                      'Accuray {acc.val:.3f} ({acc.avg:.3f})'.format(
                    i, len(val_loader), f1_local=f1_local,f1_global=f1_global, loss=losses, acc=acc))

        print(' * Accuray {acc.avg:.3f}'.format(acc=acc), '(Previous Best Acc: %.3f)' % best_precision,
              ' * F1 {f1_global:.3f}'.format(f1_global=f1_global), '(Previous Best F1: %.3f)' % best_F1,
              ' * Loss {loss.avg:.3f}'.format(loss=losses), 'Previous Lowest Loss: %.3f)' % lowest_loss)
        return acc.avg, losses.avg,f1_global
    ...
    # 保存最新模型以及最优模型
    def save_checkpoint(state, is_best, is_lowest_loss,is_best_F1, filename='./model/%s/checkpoint.pth.tar' % file_name):
        ...
        if is_best_F1:
            shutil.copyfile(filename, './model/%s/F1_best.pth.tar' % file_name)
    ...
    def float_turn_int(y_pred,topk=(1,)):
        maxk = max(topk)
        prob, pred = y_pred.topk(maxk, 1, True, True)
        return pred
    
    #处理报错 image file is truncated (13 bytes not processed)
    ...
    resume = True
    model1 = l1.net()
    model2 = model_densenet169.densenet169()
    model3 = model_inceptionv4.v4()
    ...
    model1.load_state_dict(torch.load('./tianchi_densenet201_master/model/main_densenet201/F1_best.pth.tar')['state_dict'])
    model2.load_state_dict(torch.load('./tianchi_densenet169_master/model/main_densenet169/F1_best.pth.tar')['state_dict'])
    model3.load_state_dict(torch.load('./tianchi_inceptionv4_master/model/main_inceptionv4/F1_best.pth.tar')['state_dict'])
    #设置初始精确度和损失
    best_precision = 0
    lowest_loss = 100
    best_F1=0
    ...
    if resume:
        ...
        if os.path.isfile(checkpoint_path):
            ...
            if start_epoch in np.cumsum(stage_epochs)[:-1]:
                ...
                model.load_state_dict(torch.load('./model/%s/F1_best.pth.tar' % file_name)['state_dict'])   
    ...
    for r in range(minr,maxr):
        ran=r
        print("now,r = ",r," maxr = ",maxr," minr = ",minr)
        # 程序主体
        ...
        train_data_lis, val_data_lis = train_test_split(all_data, test_size=val_ratio, random_state=ran, stratify=all_data['label'])
        #提取出训练集，验证集
        train_data_list=train_data_lis.iloc[:,0:2]
        val_data_list=val_data_lis.iloc[:,0:2]
        #将权重集转变为tensor
        train_weight=torch.Tensor(train_data_lis.iloc[:,2].values)
        val_weight=torch.Tensor(val_data_lis.iloc[:,2].values)
        #设置一个epoch的总数
        len_train=3200
        len_val=3000
        #为了处理数据不平衡，制造一个抽样的提取器
        samp_train=WeightedRandomSampler(train_weight, len_train, True)
        samp_val=WeightedRandomSampler(val_weight, len_val, True)
        ...
        # 生成图片迭代器
        train_loader = DataLoader(train_data, batch_size=batch_size, shuffle=False, sampler=samp_train, pin_memory=False, num_workers=workers)
        val_loader = DataLoader(val_data, batch_size=batch_size*2, shuffle=False, sampler=samp_val, pin_memory=False, num_workers=workers)
        test_loader = DataLoader(test_data, batch_size=batch_size*2, shuffle=False, pin_memory=False, num_workers=workers)

        # 使用交叉熵损失函数
        criterion = nn.CrossEntropyLoss().cuda()

        #原来为model.parameters()，这里改变是为了能够只训练最后一层
        ...
        if evaluate:
            ...
            for epoch in range(start_epoch, total_epochs):
                ...
                precision, avg_loss, F1  = validate(val_loader, model, criterion)
...
#                     acc_file.write('Epoch: %2d, Precision: %.8f, Loss: %.8f\n' % (epoch, precision, avg_loss))
                ...
                save_checkpoint(state, is_best, is_lowest_loss, is_best_F1)
