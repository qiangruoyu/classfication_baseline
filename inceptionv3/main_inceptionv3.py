...
def main():
        ...
        for i, (images, target) in enumerate(train_loader):
            ...
            if i % print_freq == 0:
                print('Epoch: [{0}][{1}/{2}]\t'
                    ...
                    epo, i, len(train_loader), f1_local=f1_local,f1_global=f1_global,loss=losses, acc=acc))
    ...
    # 创建inception_v3模型
    model = model_inceptionv3.inceptionv3()
    ...
    if resume:
        ...
        if os.path.isfile(checkpoint_path):
            ...
            if start_epoch in np.cumsum(stage_epochs)[:-1]:
                ...
                model.load_state_dict(torch.load('./model/%s/best_F1.pth.tar' % file_name)['state_dict'])
    ...
    for r in range(minr,maxr):
        ...
        # 生成图片迭代器
        train_loader = DataLoader(train_data, batch_size=batch_size, shuffle=False, sampler=samp_train, pin_memory=True, num_workers=workers)
...
#             time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time())), best_precision, os.path.basename(__file__)))
    ...
    F1_best_model = torch.load('./model/%s/F1_best.pth.tar' % file_name)
    model.load_state_dict(F1_best_model['state_dict'])
